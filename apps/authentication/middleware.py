...
class MFAMiddleware:
    """
    This middleware is used to globally intercept cases where MFA is enabled but not yet verified, such as OIDC, CAS, or logins done via third-party libraries that call login directly;
    it can therefore only be controlled from the Middleware
    """
    ...
    def __call__(self, request):
        ...
        if not request.session.get('auth_mfa_required'):
            return response
        # Not authenticated, meaning it did not come from a third party
        if request.user.is_anonymous:
            return response

        # This is a request needed by the mfa login page, so let it through too; the user has actually already completed login via CAS/OIDC
        white_urls = [
        ...
        # Because users logging in via CAS/OIDC who accidentally navigate to another page cannot get back
...
class ThirdPartyLoginMiddleware(mixins.AuthMixin):
    """OpenID, CAS, SAML2 login rule configuration validation"""
