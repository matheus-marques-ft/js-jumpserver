...
# API whitelist - APIs that a normal user can access
user_accessible_urls = known_unauth_urls + [
    # Add more APIs that a normal user can access
...
class Command(BaseCommand):
    ...
    error_urls = []
    unformat_urls = []
    # APIs that a user can access but that are not in the whitelist
    unexpected_access = []
    ...
    def check_user_api_access(self, urls):
        """Check APIs that a normal user can access"""
        ...
        for url, ourl in urls:
            ...
            try:
                ...
                time.sleep(0.1)
                # If the status code is 200 or 201, the user can access it
                if response.status_code in [200, 201]:
                    accessible_urls.append((url, ourl, response.status_code))
                elif response.status_code == 403:
                    # 403 means insufficient permissions, which is normal
                    pass
                else:
                    # Other status codes may indicate an error
    ...
    def check_anonymous_access(self, urls):
        """Check anonymous access permissions"""
        client = Client()
        client.defaults['HTTP_HOST'] = 'localhost'
    ...
    def check_user_access(self, urls, update_whitelist=False):
        ...
        # Check whether there are accessible APIs that are not in the whitelist
        ...
        # If the update-whitelist option is enabled
        if update_whitelist:
            print("\n=== Updating Whitelist ===")
    ...
    def handle(self, *args, **options):
        ...
        # Check anonymous access permissions (runs by default)
        if not options['skip_anonymous_check']:
            self.check_anonymous_access(urls)

        # Check user access permissions (runs by default)
